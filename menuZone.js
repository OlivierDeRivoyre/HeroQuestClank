class MenuShopZone {
    constructor(levelView) {
        ...
        this.topX = this.levelView.shopZone.topX;
...
class MenuZone {
    constructor(levelView) {
        ...
        this.endTurnButton = new Button('End turn', this.topX + 100, this.topY, 180, 40, () => levelView.endTurn());
    ...
    paint() {
        ...
        topX += 330;
        this.monsterDamageSprite.paint(topX, topY - 8);
        ...
        for (let i = 0; i < this.monsterLevelUpAt; i++) {
            screen.canvas.fillRect(i <= this.monsterLevelUpTurnCount ? '#66F' : '#BBB',
                topX + i * barWidth, topY - 8 + 32, barWidth - 2, 8);
        }
        topX += 32 + 8;
        screen.canvas.fontSize = 24;
        screen.canvas.fillStyle = '#822';
        screen.canvas.fillText(this.monsterDamage, topX, topY + textMargin);
    }
    click(mouseCoord) {
